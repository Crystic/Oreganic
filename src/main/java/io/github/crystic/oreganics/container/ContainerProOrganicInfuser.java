...
public class ContainerProOrganicInfuser extends Container {

	private TileEntityProOrganicInfuser infuser;
	private int dualCookTime;
	private int dualPower;
	@SuppressWarnings("unused")
	private int lastItemBurnTime;
	
	public ContainerProOrganicInfuser(InventoryPlayer invPlayer, TileEntityProOrganicInfuser teOrganicInfuser) {
		dualCookTime = 0;
		dualPower = 0;
		lastItemBurnTime = 0;
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int slotNum) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (slotNum < 3) {
				...
				if (TileEntityProOrganicInfuser.isOrganicInput(itemstack1)) {
					if (!this.mergeItemStack(itemstack1, 0, 1, false)) {
						return null;
					}
				} else if (TileEntityProOrganicInfuser.isMineralInput(itemstack1)) {
					if (!this.mergeItemStack(itemstack1, 1, 2, false)) {
						return null;
					}
				} else if(TileEntityProOrganicInfuser.isItemFuel(itemstack1)){
