...
import io.github.crystic.oreganics.slot.SlotOIFuel;
import io.github.crystic.oreganics.slot.SlotOIMineral;
import io.github.crystic.oreganics.slot.SlotOIOutput;
import io.github.crystic.oreganics.slot.SlotOIPlant;
...
public class ContainerBasicOrganicInfuser extends Container {

	private TileEntityBasicOrganicInfuser infuser;
	private int dualCookTime;
	private int dualPower;
	@SuppressWarnings("unused")
	private int lastItemBurnTime;
	
	public ContainerBasicOrganicInfuser(InventoryPlayer invPlayer, TileEntityBasicOrganicInfuser teOrganicInfuser) {
		dualCookTime = 0;
		dualPower = 0;
		lastItemBurnTime = 0;
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int slotNum) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (slotNum < 3) {
				...
				if (TileEntityBasicOrganicInfuser.isOrganicInput(itemstack1)) {
					if (!this.mergeItemStack(itemstack1, 0, 1, false)) {
						return null;
					}
				} else if (TileEntityBasicOrganicInfuser.isMineralInput(itemstack1)) {
					if (!this.mergeItemStack(itemstack1, 1, 2, false)) {
						return null;
					}
				} else if(TileEntityBasicOrganicInfuser.isItemFuel(itemstack1)){
