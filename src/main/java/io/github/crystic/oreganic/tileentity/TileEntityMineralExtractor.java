...
public class TileEntityMineralExtractor extends TileEntity implements
		ISidedInventory {
	...
	private static int getItemInput(ItemStack itemstack) {
		if (itemstack == null) {
			...
			if (item instanceof ItemBlock
				...
				if(block == Oreganic.cropOrganicCoal) return 1;
				if(block == Oreganic.cropOrganicIron) return 1;
				if(block == Oreganic.cropOrganicGold) return 1;
				if(block == Oreganic.cropOrganicLapis) return 1;
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				MineralExtractor.updateMineralExtractorBlockState(
